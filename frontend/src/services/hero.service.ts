...
import { HeroConfig, HeroApiResponse, HeroError } from '@/src/types/hero.types';
import { HERO_API_CONFIG, HERO_CACHE_CONFIG, DEFAULT_HERO_CONFIG } from '@/src/config/hero.config';
...
class HeroService {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (axiosError.response) {
                    throw new Error(
                        `API Error: ${axiosError.response.status} - ${JSON.stringify(axiosError.response.data) || 'Unknown error'}`
                    );
    ...
    private validateAndTransformConfig(config: Partial<HeroConfig>): HeroConfig {
        ...
        const validatedConfig: HeroConfig = {
            ...
            meta:             config.meta             ?? DEFAULT_HERO_CONFIG.meta ?? '',
        };
